...
Wind Move Keys are useful. (Actually not. Disabled for now.)
...
I've got no theme set at the moment and it seems to look okay. The only thing that I don't like is that org-babel blocks aren't visible as blocks. However, if the syntax highlighting works better then that's a win.
